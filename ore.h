...
typedef struct _ore_ciphertext{
  bool initialized;
  ore_params params;
...
typedef struct _ore_token_bit{
  element_t add_one;
  element_t minus_one;
} ore_token_bit[1];

typedef struct _ore_token{
  bool initialized;
  ore_params params;
...
typedef struct _ore_exp_bit{
  mpz_t add_one;
  mpz_t minus_one;
} ore_exp_bit[1];

typedef struct _ore_exp_tmp{
  bool initialized;
  ore_exp_bit exp_bit[PLAINTEXT_BIT];
  mpz_t k22r2;
} ore_exp_tmp[1];
...
int init_ore_token(ore_token token, ore_params params,
                   pairing_t pairing, element_t g2, element_t k22, element_t xj,  mpz_t* mim, mpz_t* mim_inv, ore_exp_tmp exp_tmp);
...
int ore_token_gen(ore_token token, uint64_t msg, pairing_t pairing, element_t k1, element_t xj,
                 element_t xi, element_t g1, element_t g2, mpz_t q, mpz_t r, mpz_t xjm_inv, mpz_t* mim, mpz_t* mim_inv, ore_exp_tmp exp_tmp);
